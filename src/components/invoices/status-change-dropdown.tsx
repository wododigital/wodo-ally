  ...
  if (invoiceType === "proforma") {
    ...
    return (
      <div className="relative">
        <button onClick={(e) => { e.stopPropagation(); setOpen((o) => !o); }} className="flex items-center gap-1 group/sd" title="Change status" aria-expanded={open} aria-haspopup="listbox" aria-label={`Change status, currently ${status}`}>
        ...
        {open && (
          <>
            ...
            <div role="listbox" aria-label="Status options" className="absolute left-0 top-full mt-1 z-20 py-1.5 rounded-xl min-w-[140px]"
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={(e) => { e.stopPropagation(); setOpen(false); }} aria-hidden="true" />
          <div role="listbox" aria-label="Status options" className="absolute left-0 top-full mt-1 z-20 py-1.5 rounded-xl min-w-[110px]"
            style={{ background: "rgba(255,255,255,0.95)", backdropFilter: "blur(16px)", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 8px 24px rgba(0,0,0,0.10)" }}>
            {options.map((s) => (
              <button key={s} role="option" onClick={(e) => { e.stopPropagation(); onChange(s); setOpen(false); }}
